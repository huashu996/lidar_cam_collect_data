...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
		...
		// 降低图像的对比度
		double contrast_factor2 = 0.8; // 调整对比度因子，较小的值会降低对比度
		src.convertTo(adjusted_image, -1, contrast_factor2, 0);
		...
		cv::normalize(channels[2], stretched_channel, 0, 255, cv::NORM_MINMAX);

		// 替换亮度通道
		stretched_channel.copyTo(channels[2]);
