...
{
    nh.param<std::string>("sub_topic", sub_topic_, "/points_ground");
    ...
    nh.param<double>("min_cluster_size", min_cluster_size_, 0.2);
    nh.param<double>("max_cluster_size", max_cluster_size_, 3);
...
void EuCluster::callback(const sensor_msgs::PointCloud2ConstPtr in)
{
    ...
    visualization_msgs::MarkerArray objs;
    visualization_msgs::MarkerArray objs2;
    ...
    for(size_t i = 0; i < objs.markers.size(); i++)
    {
        ...
        objs.markers[i].text = ' ';
        //输出平面位置投影坐标
        objs.markers[i].pose.position.z = 0;
        objs.markers[i].scale.z=0;
        std::cout <<objs.markers[i].pose<<std::endl;
        std::cout <<objs.markers[i].scale<<std::endl;
    }
    pub_.publish(objs);
